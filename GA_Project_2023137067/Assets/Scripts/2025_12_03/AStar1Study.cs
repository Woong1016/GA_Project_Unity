...
public class AStar1Study : MonoBehaviour
{
    public int width = 21;
    public int height = 21;
    [Range(0, 100)] public int wallPercent = 20;
    ...
    int GetHeuristic(Vector2Int a, Vector2Int b)
    {
        ...
        for (int x = -1; x <= 1; x++)
        {
            for (int y = -1; y <= 1; y++)
            {
                ...
                if (nx < 0 || ny < 0 || nx >= width || ny >= height || map[nx, ny] == 0)
                {
                    isNearWall = true;
                    break;
                }
    ...
    void DrawMap()
    {
        ...
        for (int x = 0; x < width; x++)
        {
            for (int y = 0; y < height; y++)
            {
                ...
                if (x == startPos.x && y == startPos.y) rend.material.color = Color.blue;
                if (x == goalPos.x && y == goalPos.y) rend.material.color = Color.red;
            }
        }
    }
